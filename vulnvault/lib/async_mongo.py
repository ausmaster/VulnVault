"""
Provides Specialized AsyncMongoClient that works with VaultConfig.
"""
...
class AsyncVaultMongoClient(AsyncMongoClient):
    """A specific AsyncMongoClient dedicated to working with VaultConfig."""
    db: AsyncDatabase
    ...
    async def ensure_indexes(self) -> None:
        ...
        await self.cpes.create_index("cpe_name", unique=True)
        await self.cpematches.create_index("matches")
        await self.cves.create_index("configurations.nodes.cpeMatch.matchCriteriaId")
    ...
    async def raise_if_not_connected(
        ...
        """Ensures connectivity. Returns self if successful, else raises ConnectionFailure.

        Args:
            exception_str: Custom exception string to raise.
